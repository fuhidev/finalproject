...
public class TextJsoup {
	private String url;
	private String cssQuery;
	
	
	public TextJsoup(String url, String cssQuery) {
		super();
		this.url = url;
	...
	public void getData() throws IOException{
		...
		ArrayList<JsonDB> rs     = new ArrayList<JsonDB>();
		int count = 0;
		tmpExp = this.exportDB();
		for(int i = 0; i < tmpExp.size()-1; i++){
			count++;
			if(i != tmpExp.size()-2){
				JsonDB js = new JsonDB(count,tmpExp.get(i), Double.parseDouble(tmpExp.get(i+1).replace(",","")),Double.parseDouble(tmpExp.get(i+2).replace(",","")));
				rs.add(js);
				i+=2;
			}
	...
	public static void main(String[] args) throws IOException {
		TextJsoup jsoup = new TextJsoup("http://vip.giavang.net/teline2/data/banggiavang1a.php", "div#myDiv table:gt(0) tbody tr:gt(1) td font");
		jsoup.getData();
	}
}
